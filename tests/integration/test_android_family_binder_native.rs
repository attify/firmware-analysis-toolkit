...
#[test]
fn binder_native_boundary_emits_lead_for_binder_to_native_parser() {
    let dir = tempdir().expect("tempdir");
    let base_apk = dir.path().join("base.apk");
    write_manifest_apk(&base_apk, "com.example.binder");
    ...
    assert_eq!(report["leads"][0]["family"], "binder-native-boundary");
    assert!(report["leads"][0]["why_matched"]
        .as_array()
        .expect("why matched")
        .iter()
        .any(|value| value == "binder input reaches size-sensitive native boundary"));
}

#[test]
fn binder_native_boundary_suppresses_binder_without_native_evidence() {
    let dir = tempdir().expect("tempdir");
    let base_apk = dir.path().join("base.apk");
    write_manifest_apk(&base_apk, "com.example.binder.safe");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_fat"))
        .args([
            ...
            base_apk.to_str().expect("apk path"),
            "--semantic-bundle",
            semantic_fixture("binder-native-negative.json")
                .to_str()
                .expect("bundle path"),
            "--family",
            "binder-native-boundary",
