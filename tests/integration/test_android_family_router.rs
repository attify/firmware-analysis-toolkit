use serde_json::Value;
use std::path::PathBuf;
use std::process::Command;
use tempfile::tempdir;
use zip::write::SimpleFileOptions;
use zip::ZipWriter;

fn write_manifest_apk(path: &std::path::Path, package_name: &str) {
    let file = std::fs::File::create(path).expect("create apk");
    let mut zip = ZipWriter::new(file);
    let opts = SimpleFileOptions::default();
    let manifest = format!(r#"<manifest package="{package_name}"><application /></manifest>"#);

    zip.start_file("AndroidManifest.xml", opts)
        .expect("manifest entry");
    use std::io::Write as _;
    zip.write_all(manifest.as_bytes()).expect("manifest bytes");
    zip.start_file("classes.dex", opts).expect("classes.dex");
    zip.write_all(b"dex").expect("dex bytes");
    zip.finish().expect("finish zip");
}

fn semantic_fixture(name: &str) -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/fixtures/android/semantic")
        .join(name)
}

#[test]
fn remote_router_gadget_emits_explicit_router_lead() {
    let dir = tempdir().expect("tempdir");
    let base_apk = dir.path().join("base.apk");
    write_manifest_apk(&base_apk, "com.example.app");

    let output = Command::new(env!("CARGO_BIN_EXE_fat"))
        .args([
            "android",
            "discover",
            "--apk",
            base_apk.to_str().expect("apk path"),
            "--semantic-bundle",
            semantic_fixture("router-mini.json")
                .to_str()
                .expect("bundle path"),
            "--family",
            "remote-router-gadget",
            "--json",
        ])
        .output()
        .expect("android discover runs");

    assert!(output.status.success(), "{output:?}");
    let report: Value = serde_json::from_slice(&output.stdout).expect("json");
    assert_eq!(report["status"], "partial-family-ready");
    assert_eq!(report["lead_count"], 1);
    assert_eq!(report["leads"][0]["family"], "remote-router-gadget");
    assert_eq!(report["leads"][0]["symbol"], "com.example.Router.dispatch");
    assert!(report["leads"][0]["why_matched"]
        .as_array()
        .expect("why matched")
        .iter()
        .any(|value| value == "router dispatch accepts external route material"));
}

#[test]
fn remote_router_gadget_emits_path_composition_lead_for_synthetic_bundle() {
    let dir = tempdir().expect("tempdir");
    let base_apk = dir.path().join("base.apk");
    write_manifest_apk(&base_apk, "com.example.remotecontrol");

    let output = Command::new(env!("CARGO_BIN_EXE_fat"))
        .args([
            "android",
            "discover",
            "--apk",
            base_apk.to_str().expect("apk path"),
            "--semantic-bundle",
            semantic_fixture("remote-control.json")
                .to_str()
                .expect("bundle path"),
            "--family",
            "remote-router-gadget",
            "--json",
        ])
        .output()
        .expect("android discover runs");

    assert!(output.status.success(), "{output:?}");
    let report: Value = serde_json::from_slice(&output.stdout).expect("json");
    assert_eq!(report["status"], "partial-family-ready");
    assert_eq!(report["lead_count"], 1);
    assert_eq!(
        report["leads"][0]["symbol"],
        "/api/v1/devices/{id}/module/status"
    );
    assert!(report["leads"][0]["score_trace"]["family_pack_hits"]
        .as_array()
        .expect("family hits")
        .iter()
        .any(|value| value == "path-composition-owned-by-remote-router"));
}

#[test]
fn remote_router_gadget_suppresses_shallow_synthetic_bundle() {
    let dir = tempdir().expect("tempdir");
    let base_apk = dir.path().join("base.apk");
    write_manifest_apk(&base_apk, "com.example.shallowcontrol");

    let output = Command::new(env!("CARGO_BIN_EXE_fat"))
        .args([
            "android",
            "discover",
            "--apk",
            base_apk.to_str().expect("apk path"),
            "--semantic-bundle",
            semantic_fixture("shallow-control.json")
                .to_str()
                .expect("bundle path"),
            "--family",
            "remote-router-gadget",
            "--json",
        ])
        .output()
        .expect("android discover runs");

    assert!(output.status.success(), "{output:?}");
    let report: Value = serde_json::from_slice(&output.stdout).expect("json");
    assert_eq!(report["status"], "partial-family-ready");
    assert_eq!(report["lead_count"], 0);
}
