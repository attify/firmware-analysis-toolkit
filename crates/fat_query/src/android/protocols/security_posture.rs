...
pub(crate) fn derive_security_posture_semantics(
    index: &ProtocolIndex,
...
fn signal_fact_kind(kind: ProtocolSignalKind) -> Option<&'static str> {
    match kind {
        ProtocolSignalKind::HostnameVerificationDisabled => {
            Some("tls.hostname-verification-disabled")
        }
        ProtocolSignalKind::WeakTrustManager => Some("tls.weak-trust-manager"),
        ProtocolSignalKind::PlaintextTokenStore => Some("storage.plaintext-token-store"),
        ProtocolSignalKind::AvailableEncryptionUnused => {
            Some("storage.available-encryption-unused")
        }
        _ => None,
    }
}
